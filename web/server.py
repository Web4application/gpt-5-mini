...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        # Keep connection open; no messages expected from client
        while True:
...
# Background task: watch history.json and broadcast updates
async def watch_history():
    os.makedirs(STORIES_DIR, exist_ok=True)
    async for changes in awatch(STORIES_DIR):
        # We only care if history.json changes
        for _, path in changes:
            if os.path.basename(path) == "history.json":
                try:
                    with open(HISTORY_FILE, "r", encoding="utf-8") as f:
...
async def startup_event():
    asyncio.create_task(watch_history())
